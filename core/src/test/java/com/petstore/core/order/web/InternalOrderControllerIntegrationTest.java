...
/**
 * Characterizes the fulfillment shipment callback: token-gated, qtyShipped-guarded
 * (replays are no-ops), and driving APPROVED -> PARTIALLY_SHIPPED -> COMPLETED
 * through the state machine. Ordered: one order is walked through its lifecycle.
 */
@Testcontainers
...
class InternalOrderControllerIntegrationTest {
	...
	void partialShipmentMovesAnApprovedOrderToPartiallyShipped() throws Exception {
		...
		placeSmallApprovedOrder(j2ee, 2); // 1001: EST-1 x2, auto-approved

		mockMvc.perform(post("/api/internal/orders/1001/shipments")
						.header(TOKEN_HEADER, DEV_TOKEN)
						.contentType(MediaType.APPLICATION_JSON)
						.content("{\"shipments\":[{\"itemId\":\"EST-1\",\"qtyShipped\":1}]}"))
				.andExpect(status().isOk());

		mockMvc.perform(get("/api/orders/1001").header("Authorization", "Bearer " + j2ee))
				.andExpect(status().isOk())
	...
	void replayingACallbackOnACompletedOrderIsANoOp() throws Exception {
		...
		mockMvc.perform(get("/api/orders/1001").header("Authorization", "Bearer " + j2ee))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.status").value("COMPLETED"))
				.andExpect(jsonPath("$.lines[0].qtyShipped").value(2)) // still qty, not qty+1
				.andExpect(jsonPath("$.statusHistory.length()").value(4)); // no extra entries
	}
