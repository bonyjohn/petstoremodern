...
`core` is a modular monolith: business modules (`catalog`, `customer`, `order`, `notification`) may share `common` but never each other — enforced by ArchUnit. `fulfillment-service` is a real extraction: it shares no code with core, only the database collections and the callback contract. Orders reaching `APPROVED` are picked up via a change stream (resume-token checkpointed, at-least-once, single active consumer behind a TTL'd lease), inventory is decremented, and shipped quantities are reported back to core, which advances the order to `PARTIALLY_SHIPPED`/`COMPLETED`. The stream is the latency path; a reconciliation sweep (startup, lost-token, periodic) is the delivery guarantee — it also ships backorders once stock returns. The data lives in 7 domain collections (`categories`, `products`, `items`, `customers`, `orders`, `counters` owned by core; `inventory` owned by fulfillment) plus 1 infrastructure collection (`fulfillment_checkpoints`: the change-stream checkpoint and the consumer lease).
...
- **Pipeline resilience tests**: the fulfillment consumer is integration-tested for the failure modes that matter — kill-and-restart resumes from its checkpoint and catches up on orders approved while down; replayed deliveries never double-decrement inventory; the reconciliation sweep ships orders approved before the service's first-ever start and orders stalled on empty stock once restocked; and the consumer lease keeps a second instance out until the holder releases (at-least-once + idempotency + takeover, asserted, not assumed).
